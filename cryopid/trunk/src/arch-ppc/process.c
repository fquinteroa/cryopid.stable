...
static int save_registers(pid_t pid, struct user* r)
{
    int i;
    long* l = (long*)r;
    ...
    /* FIXME: FP & VMX registers too! */
    return 0;
}

static int restore_registers(pid_t pid, struct user* r)
{
    int i;
    long* l = (long*)r;
    for (i = 0; i < sizeof(*r)/sizeof(long); i++) {
	errno = 0;
	if (i == 34) /* PT_ORIG_R3 can't be set */
	    continue;
	if (i == 40) /* skip the emptiness here */
	    i = PT_FPR0; /* resume from FP regs (48) */
	if (ptrace(PTRACE_POKEUSER, pid, i*sizeof(long), l[i]) < 0) {
	    debug("ptrace setregs (%d), ", i);
...
int is_in_syscall(pid_t pid, struct user *user)
{
    ...
    if (user->regs.nip >= vdso_start && user->regs.nip < vdso_end)
	return 1;

    inst = ptrace(PTRACE_PEEKDATA, pid, user->regs.nip-4, 0);
    if (errno) {
...
static inline unsigned long __remote_syscall(pid_t pid,
	...
	int use_r3, unsigned long r3,
	int use_r4, unsigned long r4,
	int use_r5, unsigned long r5,
	int use_r6, unsigned long r6,
	int use_r7, unsigned long r7)
{
    struct user orig_regs, regs;
    unsigned long ret;
    int status;
    ...
    regs.regs.gpr[0] = syscall_no;
    if (use_r3) regs.regs.gpr[3] = r3;
    if (use_r4) regs.regs.gpr[4] = r4;
    if (use_r5) regs.regs.gpr[5] = r5;
    if (use_r6) regs.regs.gpr[6] = r6;
    if (use_r7) regs.regs.gpr[7] = r7;

    /* Set up registers for ptrace syscall */
    regs.regs.nip = syscall_loc;
    ...
    if (regs.regs.ccr & 0x10000000) {
	errno = regs.regs.gpr[3];
	return -1;
    }

    return regs.regs.gpr[3];
}
