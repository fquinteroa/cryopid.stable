...
{
    char *cp, *code = (char*)TRAMPOLINE_ADDR(index);
    ...
    *(unsigned short*)(cp) = r->cs; cp+= 2; /* jmp cs:foo */
    ...
    while (atomic_read(THREAD_COUNTER) >= 0);
    asm("jmp *%%eax\n" : : "a"(TRAMPOLINE_ADDR(index)));
}

static void load_chunk_regs(struct user *user, int stopped, int index)
{
    char *cp, *code = (char*)TRAMPOLINE_ADDR(index);
    struct user_regs_struct *r = &user->regs;
    ...
    *(unsigned short*)(cp) = r->cs; cp+= 2; /* jmp cs:foo */
    syscall_check(
	(int)mprotect((void*)TRAMPOLINE_ADDR(index), PAGE_SIZE, PROT_READ|PROT_EXEC),
	    0, "mmap");
}
...
void read_chunk_regs(void *fptr, int action)
{
	...
	fprintf(stderr, "(registers): Thread was %sstopped\n",
		stopped?"":"not ");
	...
	load_chunk_regs(&user, stopped, 0);
}
...
struct user *read_chunk_regs_noload(void *fptr, int action)
{
    ...
    user = xmalloc(sizeof(struct user));
    read_bit(fptr, user, sizeof(struct user));
    read_bit(fptr, &stopped, sizeof(int));
